...
- **Wed (Apr 22) — done:**
  - ✅ **ADK Go spike** — pulled forward from Fri Apr 24. Throwaway `daemon/cmd/spike-adk/` binary runs an ADK-Go `llmagent` + one stub tool end-to-end against local LM Studio (Gemma 4 e4b). Anthropic adapter wired into `go.mod` but unused in the spike run — switching providers is a line change. Deps landed: `google.golang.org/adk` v1.1.0, `huytd/adk-openai-go`, `Alcova-AI/adk-anthropic-go` v0.1.15, `google.golang.org/genai` v1.54.0. Hard stop cleared two days early.
  - ✅ **PRD corrections** — §11.1 / §11.10 module path fixed (`google.golang.org/adk`, not `github.com/google/adk-go`), provider-adapter caveat added.
...
| Risk | Mitigation |
|---|---|
| **Phase 1 is deliberately over-scoped.** | This is the biggest risk. If Phase 1 slips, Phase 4 (testing) is the absorber — not Phase 5 (demo). Original hard stops: ADK Go spike working by Fri Apr 24; MCP client by Mon Apr 27. **Current state (Apr 22):** ADK spike cleared two days early; MCP client pulled forward to Thu Apr 23 with a Fri–Mon slip window. If Marketing agent misses Tue Apr 28, cut its scope to a hard-coded proposal (skip the LLM) so kanban still demos a real issue flow. |
...
| **ADK Go unfamiliarity for Elizabeth.** | **Resolved Wed Apr 22** — spike landed two days ahead of plan against local LM Studio (Gemma 4 e4b); `llmagent` + stub `functiontool` + runner loop all green on first run. Third-party adapters (`Alcova-AI/adk-anthropic-go`, `huytd/adk-openai-go`) wired into `daemon/go.mod`; provider-adapter caveat captured in PRD §11.10. |
